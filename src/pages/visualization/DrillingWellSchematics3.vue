...
onMounted(async () => {
  ...
  // Rectangle center and dimensions
    ...
    const width = 20; // along X
    const height = 50; // along Y
    //const angle = (40 * Math.PI) / 180; // convert degrees to radians
    const angle = (-40 * Math.PI) / 180; // negative for clockwise rotation
    ...
    const layout = {
    title: '100 × 100 axes (Y inverted) with tilted rectangle',
    xaxis: {
        range: [0, 100],
        zeroline: true,
        mirror: true,
