...
import { VLPSENSITIVITYVARIABLES } from '../../../../constants/asset_models';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const drainagePointId = ref(props.selectedComponent.assetId);
    const assetGroupId = ref(props.selectedComponent.assetGroupId);

    console.log("props.selectedComponent: ", props.selectedComponent)
    console.log("props.selectedComponent.assetId: ", props.selectedComponent.assetId)
    console.log("props.selectedComponent.assetGroupId: ", props.selectedComponent.assetGroupId)
    console.log("drainagePointId: ", drainagePointId)
    console.log("assetGroupId: ", assetGroupId)

    const vlpSensitivityVariables = ref(VLPSENSITIVITYVARIABLES)
    ...
    const calculateVLPCurve = async () => {
      ...
      const vlpCalculatorDto = {
        drainagePointId: drainagePointId.value,
    ...
    const updateSelectedLiftCurves = (responseData) => {

        liftCurvesRecord.value = {...responseData};
        const liftCurvesKeys = Object.keys(responseData.liftCurves);
        ...
        const senVar1Name = responseData.SensitivityVariable1Name;
        const senVar2Name = responseData.SensitivityVariable2Name;
        const senVar3Name = responseData.SensitivityVariable3Name;
    ...
    const onCurveChange = async (curve) => {
      ...
      emit('updateActiveAssetInput', {
        ...
        variable: "liftCurveName",
        modelName: "VLP"
      });
      
      const selectedLiftCurvesResponse = await findLiftCurvesNamesPerDrainagePoint(payload);
        if(selectedLiftCurvesResponse.status == 200 || selectedLiftCurvesResponse.status == 201){

            if(selectedLiftCurvesResponse.data){
                updateSelectedLiftCurves(selectedLiftCurvesResponse.data);
            }
        }
    }
    ...
    const getLiftCurvesNamesPerDrainagePoint= async () =>{
        ...
        if(liftCurvesNamesResponse.status == 200 || liftCurvesNamesResponse.status == 201){
            ...
            if(liftCurvesNamesResponse.data.firstLiftCurvesRecord){
                updateSelectedLiftCurves(liftCurvesNamesResponse.data.firstLiftCurvesRecord);
            }
        }
    ...
    watch(
        ...
        async (newSelectedComponent) => {
            assetGroupId.value = newSelectedComponent.assetGroupId;
            drainagePointId.value = newSelectedComponent.assetId;
